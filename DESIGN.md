...
Iteration Guide
Focus on a single component at a time. Reference YAML keys directly.
New CTAs default to `{rounded.pill}` (100px); new icon plates default to `{rounded.full}`. Cards use `{rounded.xl}`.
Variants live as separate entries inside the `components:` block.
Use `{token.refs}` everywhere — never inline hex.
Hover state never documented. Only Default and Active/Pressed.
CoinbaseDisplay 400 for display, CoinbaseSans 400/600/700 for body. CoinbaseMono on every number.
Coinbase Blue stays scarce — one or two blue moments per band.
Known Gaps
